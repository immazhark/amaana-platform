...
CMD ["sh", "-c", "node prisma/migrate-deploy-with-retry.mjs && node prisma/import-reviewed-campaigns.mjs && if [ \"$APP_ENVIRONMENT\" = \"staging\" ]; then node prisma/run-db-script-with-retry.mjs prisma/seed.mjs && node prisma/run-db-script-with-retry.mjs prisma/seed-staging-acceptance.mjs; fi && if [ \"$APP_ENVIRONMENT\" = \"staging\" ] && [ \"$PUBLIC_MEDIA_ACCEPTANCE_ON_START\" = \"true\" ]; then npm run acceptance:public-media; fi && if [ \"$APP_ENVIRONMENT\" = \"staging\" ] && [ \"$STAGING_ACCEPTANCE_ON_START\" = \"true\" ]; then node prisma/staging-start-with-acceptance.mjs; else node server.js; fi"]
