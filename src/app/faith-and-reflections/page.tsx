...
export default async function FaithAndReflectionsPage() {
  ...
  return (
    <div className="v2-home v2-faith-page">
      ...
      <section className="v2-section paper v2-faith-topics"><div className="v2-shell"><div className="v2-section-head"><div><p className="v2-section-label">Themes</p><h2 className="v2-section-title">A library that grows with meaning.</h2></div><p className="v2-section-intro">Themes appear as reviewed reflections are published, helping related ideas stay connected without forcing content into artificial categories.</p></div>{topics.length > 0 ? <div className="v2-faith-topic-cloud">{topics.map(([slug,name],index)=><span key={slug}><b>{String(index+1).padStart(2,"0")}</b>{name}</span>)}</div> : <p className="v2-section-intro">Topics will appear when reviewed Faith content is published.</p>}</div></section>
