...
export default async function StoriesPage() {
  ...
  return (
    <div className="v2-home v2-stories-page">
      ...
      <section className="v2-section paper v2-stories-archive" aria-labelledby="stories-archive-title"><div className="v2-shell"><div className="v2-section-head"><div><p className="v2-section-label">Living archive</p><h2 className="v2-section-title" id="stories-archive-title">Circumstance. Verification. Action. Outcome.</h2></div><p className="v2-section-intro">Completed work remains here so its story can still be understood after an appeal closes or a distribution ends.</p></div>
        {remainingStories.length > 0 ? <div className="v2-stories-grid">{remainingStories.map((story,index)=>{const identity=selectIdentityPublicImage(story.mediaAssets);return <Link className={`v2-stories-card ${index%5===0?"wide":""}`} href={`/stories/${story.slug}`} key={story.id} aria-label={`Read ${story.title}`}><div className="v2-stories-card-media">{identity ? <PublicMedia asset={identity} /> : <WorkVisualPlaceholder label={story.title} />}</div><div className="v2-stories-card-copy"><span>{String(index+2).padStart(2,"0")}</span><small>{story.initiative?.title ?? story.cause?.title ?? "Story of Amanah"}</small><h3>{story.title}</h3><p>{story.summary}</p><b>Read story ↗</b></div></Link>})}</div> : leadStory ? <div className="v2-stories-single-note"><span>Archive beginning</span><p>More stories will appear here when they are ready to be shared responsibly.</p></div> : <div className={styles.emptyState}><div className={styles.emptyMark} aria-hidden="true">✦</div><div className={styles.emptyCopy}><span className="v2-reminder-label">Privacy protected archive</span><h3>No public stories are available yet.</h3><p>Stories appear only after they are ready for public sharing. Until then, the people and details behind them remain private.</p><Link className="v2-text-link" href="/our-work">Explore published initiatives →</Link></div></div>}
      </div></section>

      <section className="v2-section dark v2-stories-ethic"><div className="v2-shell v2-stories-ethic-grid"><div><p className="v2-section-label">Editorial ethic</p><h2 className="v2-section-title">Dignity comes before visibility.</h2><p className="v2-section-intro">Amaana&apos;s public storytelling exists to explain the work, not to turn vulnerability into content.</p></div><div className={`v2-stories-ethic-list ${styles.ethicList}`}><article><span>01</span><h3>Share only what is approved</h3><p>Stories and media are reviewed separately before anything is shared publicly.</p></article><article><span>02</span><h3>Keep private proofs private</h3><p>Identity documents, medical records and sensitive verification material do not belong in the public story.</p></article><article><span>03</span><h3>Separate fact from inference</h3><p>We share what is known, and do not present assumptions about recovery, fulfilled needs or personal circumstances as fact.</p></article></div></div></section>
