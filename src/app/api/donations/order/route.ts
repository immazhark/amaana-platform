...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "Please check the donation information." }, { status: 400 });
    const appeal = await prisma.appeal.findFirst({ where: { id: parsed.data.appealId, status: "PUBLISHED" }, select: { id: true, title: true } });
    if (!appeal) return NextResponse.json({ error: "This appeal is not accepting donations." }, { status: 409 });
    ...
    return NextResponse.json({ donationId: donation.id, orderId: order.id, amount: amountPaise, currency: "INR", keyId: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID, appealTitle: appeal.title, donor: { name: donation.donorName, email: donation.donorEmail, contact: donation.donorPhone }, receiptToken }, { status: 201, headers: { "Cache-Control": "no-store" } });
  } catch (error) { console.error("Donation order creation failed", error); return NextResponse.json({ error: "We could not start the secure payment. Please try again." }, { status: 500 }); }
}
