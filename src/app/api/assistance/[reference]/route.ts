...
export async function GET(request: Request, { params }: Props) {
  ...
  if (!record || supplied.length !== expected.length || !timingSafeEqual(supplied, expected)) return NextResponse.json({ error: "Request not found." }, { status: 404 });
  return NextResponse.json({ referenceNumber: reference, status: record.status, submittedAt: record.createdAt, updatedAt: record.updatedAt }, { headers: { "Cache-Control": "no-store, private" } });
}
