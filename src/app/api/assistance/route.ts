...
export async function POST(request: Request) {
  try {
    ...
    if (files.length > MAX_FILES) return NextResponse.json({ error: `You can upload up to ${MAX_FILES} documents.` }, { status: 400 });
    ...
    return NextResponse.json({ referenceNumber, trackingToken }, { status: 201, headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    console.error("Assistance submission failed", error);
    return NextResponse.json({ error: "We could not securely submit your request. Please try again later." }, { status: 500 });
  }
}
