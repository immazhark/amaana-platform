...
export default async function InitiativePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="v2-home campaign-page">
      ...
      <section className="campaign-story v2-shell" id="campaign-story"><div><h2>What happened</h2>{initiative.primaryMetric && <div className="campaign-outcome"><strong>{initiative.primaryMetric}</strong><p>{initiative.primaryMetricLabel}</p></div>}</div><div className="campaign-story-copy">{paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div></section>

      {gallery.length > 0 && <section className="campaign-gallery" id="campaign-gallery"><div className="v2-shell"><div className="campaign-section-heading"><h2>From the drive</h2><p>Original photographs, privacy-protected videos and campaign updates from this edition.</p></div><div className="campaign-gallery-grid">{gallery.map(asset => { const url = resolvePublicMediaUrl(asset); return asset.kind === "IMAGE" && url ? <a className="campaign-image-link" href={url} target="_blank" rel="noopener noreferrer" key={asset.id} aria-label={`View full image: ${asset.altText} (opens in a new tab)`}><PublicMedia asset={asset} /><span className="campaign-open">View full image</span></a> : <PublicMedia asset={asset} key={asset.id} />; })}</div></div></section>}
