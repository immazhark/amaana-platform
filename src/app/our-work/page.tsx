  ...
  return (
    <div className="v2-home v2-work-index">
      ...
      <section className="v2-section paper" id="work-results"><div className="v2-shell"><div className="v2-section-head"><div><p className="v2-section-label">Explore by need</p><h2 className="v2-section-title">A living portfolio of service.</h2></div><p className="v2-section-intro">The five umbrella programmes stay consistent across the site. Parent programmes appear once on the main index; use the year filter when you want to inspect a specific annual edition.</p></div>
        <form className="work-filters" action="/our-work#work-results" method="get" aria-label="Filter published initiatives"><label htmlFor="work-programme">Programme<select id="work-programme" name="programme" defaultValue={filters.programme}><option value="">All programmes</option>{filters.programme && !visibleCauses.some(cause => cause.slug === filters.programme) && <option value={filters.programme}>Unavailable programme</option>}{visibleCauses.map(cause => <option key={cause.slug} value={cause.slug}>{cause.title}</option>)}</select></label><label htmlFor="work-year">Year<select id="work-year" name="year" defaultValue={filters.year}><option value="">All years</option>{filters.year && !filters.years.some(year => String(year) === filters.year) && <option value={filters.year}>Unavailable year</option>}{filters.years.map(year => <option key={year} value={year}>{year}</option>)}</select></label><button className="v2-button" type="submit">Apply filters</button>{filters.active && <Link href="/our-work#work-results">Clear filters</Link>}</form>
