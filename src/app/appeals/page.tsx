...
export default async function AppealsPage() {
  ...
  const isSynthetic = (appeal: { slug: string; title: string }) => appeal.slug.startsWith("staging-") || appeal.title.startsWith("STAGING TEST");
  const appeals = appealRecords.filter(appeal => isAppealOpenForDonations(appeal) && (isStaging || !isSynthetic(appeal)));
  const completedAidMedia = new Map(completedAid.map(item => [item.slug, item.mediaAssets[0] ?? null]));
