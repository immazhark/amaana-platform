...
export default async function AppealDetailPage({ params }: Props) {
  ...
  return <><section className="page-hero"><div className="container"><span className="tag">{appeal.category.replaceAll("_", " ")}</span><h1 style={{marginTop: "1rem"}}>{appeal.title}</h1><p className="lead">{appeal.summary}</p></div></section><section className="section"><div className="container detail-layout"><article><p className="eyebrow">The need</p><h2>{appeal.beneficiaryDisplayName ?? "A verified request for support"}</h2><div className="appeal-story">{appeal.story}</div>{appeal.updates.length > 0 && <section className="appeal-updates"><h2>Updates</h2>{appeal.updates.map(update => <article className="card" key={update.id}><p className="eyebrow">{update.publishedAt?.toLocaleDateString("en-IN", { dateStyle: "long" })}</p><h3>{update.title}</h3><p>{update.content}</p></article>)}</section>}<h3>Privacy and dignity</h3><p>Only information approved for public sharing is shown. Supporting documents remain private.</p></article><aside className="card donation-panel"><p className="eyebrow">Appeal progress</p><h2>{formatINR(raised)}</h2><p>raised of {formatINR(goal)}</p><div className="progress" aria-label={`${progress}% funded`}><span style={{width: `${progress}%`}} /></div><p className="muted">{progress}% funded</p><button className="button" type="button" style={{width: "100%"}} disabled={appeal.status !== "PUBLISHED"}>Donate to this appeal</button><p className="muted" style={{fontSize: ".85rem", marginTop: "1rem"}}>Domestic donations only. Payment processing will be enabled in a later release.</p></aside></div></section></>;
}
