...
import { getFirstPublicationIssues, goalMatchesApprovedPublicTarget } from "@/lib/appeal-publication";
...
export async function transitionAppeal(formData: FormData) {
  ...
  const publicationIssues = getFirstPublicationIssues({ fromStatus: current.status, toStatus: next, goalAmount: current.goalAmount, verification: current.assistanceRequest?.verification });
