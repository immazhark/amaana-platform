...
export async function createAppeal() {
  ...
  const appeal = await prisma.appeal.create({ data: { slug: `draft-${Date.now().toString(36)}`, title: "Untitled appeal", summary: "Complete this draft before submitting it for review.", story: "Add the verified public story here.", category: "OTHER", beneficiaryName: "Pending", goalAmount: 1, createdById: user.id } });
  await prisma.auditEvent.create({ data: { actorId: user.id, action: "appeal.created", entityType: "Appeal", entityId: appeal.id } });
  redirect(`/admin/appeals/${appeal.id}`);
}
...
export async function updateAppeal(formData: FormData) {
  ...
  const category = String(formData.get("category")); const coverImageValue = String(formData.get("coverImageUrl") || "").trim();
  ...
  const current = await prisma.appeal.findUniqueOrThrow({ where: { id } }); if (!["DRAFT", "UNDER_REVIEW", "REJECTED"].includes(current.status)) throw new Error("Published appeal content cannot be edited in this workflow");
  await prisma.$transaction([prisma.appeal.update({ where: { id }, data: { title, slug, summary, story, goalAmount, category: category as AppealCategory, beneficiaryDisplayName: String(formData.get("beneficiaryDisplayName") || "").trim() || null, beneficiaryLocation: String(formData.get("beneficiaryLocation") || "").trim() || null, coverImageUrl: coverImageValue || null } }), prisma.auditEvent.create({ data: { actorId: user.id, action: "appeal.content_updated", entityType: "Appeal", entityId: id } })]);
...
export async function transitionAppeal(formData: FormData) {
  ...
  const current = await prisma.appeal.findUniqueOrThrow({ where: { id } }); if (!transitions[current.status].includes(next)) throw new Error("Invalid appeal status transition");
