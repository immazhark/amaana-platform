...
export function DonationForm({ appealId, appealTitle }: { appealId: string; appealTitle: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const checkout = new window.Razorpay({ key: order.keyId, amount: order.amount, currency: order.currency, name: "Amaana Foundation", description: appealTitle, order_id: order.orderId, prefill: order.donor, theme: { color: "#466FAA" }, modal: { ondismiss: () => setBusy(false) }, handler: async payment => {
  ...
  return <><Script src="https://checkout.razorpay.com/v1/checkout.js" strategy="lazyOnload" onLoad={() => setScriptReady(true)} onError={() => setError("Secure checkout could not load. Please refresh and try again.")}/><form className="v2-premium-form v2-donation-form" onSubmit={submit}>
    ...
    <div className="form-grid">
      <div className="field full v2-amount-field"><label htmlFor="amount">Donation amount <span>INR</span></label><div className="v2-amount-input"><b>₹</b><input id="amount" name="amount" type="number" min="10" max="1000000" step="1" inputMode="numeric" placeholder="Enter amount" required/></div></div>
