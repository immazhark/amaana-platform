...
export function AssistanceForm() {
  ...
  return <form ref={formRef} className={`v2-premium-form v2-assistance-form ${styles.form}`} onSubmit={submit} encType="multipart/form-data" noValidate aria-busy={submitting} aria-labelledby="assistance-form-heading" aria-describedby="assistance-form-description">
    ...
    <nav className={styles.progress} aria-label="Assistance request progress">
      <p aria-live="polite" aria-atomic="true"><strong>Step {step} of 4</strong><span>{stepLabels[step - 1]}</span></p>
      <ol>{stepLabels.map((label, index) => { const itemStep = (index + 1) as AssistanceStep; return <li key={label}><button type="button" onClick={() => navigateToStep(itemStep)} disabled={itemStep > maxVisitedStep || submitting} aria-current={itemStep === step ? "step" : undefined} aria-controls={`assistance-step-${itemStep}`}><span>{String(itemStep).padStart(2, "0")}</span>{label}</button></li>; })}</ol>
    </nav>
    {error && <div ref={errorRef} className="form-error" role="alert" aria-live="assertive" tabIndex={-1}>{error}</div>}
    <fieldset id="assistance-step-1" data-assistance-step="1" tabIndex={-1} hidden={step !== 1}><legend><span>01</span> Contact</legend><div className="form-grid">
    ...
    <fieldset id="assistance-step-2" data-assistance-step="2" tabIndex={-1} hidden={step !== 2}><legend><span>02</span> Need</legend><div className="form-grid">
